...
function PreviewContent() {
    ...
    const loadData = async () => {
      try {
        ...
        if (saved) {
          ...
          let session = normalizedProject.currentSessionId
            ? normalizedProject.sessions.find(
  ...
  useEffect(() => {
    const handleFocus = async () => {
      ...
      if (saved) {
        ...
        let session = saved.currentSessionId
  ...
  const togglePageExport = async (pageId: string) => {
    ...
    try {
      await storageService.markPageForExport(
        project.id,
        currentSession.id,
        pageId,
        !currentSession.pages.find((p) => p.id === pageId)?.markedForExport,
      );
    } catch (err) {
        ...
        await new Promise((resolve, reject) => {
          img.onload = () => {
            try {
              ...
              const processedSrc =
                pdfQuality === 'low' ? resizeImage(img, 'low') : img.src;
        ...
        <div className="max-w-4xl mx-auto space-y-12 export-container print:max-w-full print:space-y-0">
          <div className="flex flex-col md:flex-row md:justify-between md:items-center print:hidden border-b border-zinc-200 pb-6 mb-8 gap-4">
            <div>
              ...
              <p className="text-zinc-600 text-xs mt-1 font-manga">
                {currentSession
                  ? `${currentSession.pages.length} total images | `
                  : ''}
            ...
            <div className="flex gap-3 flex-wrap items-center">
              ...
              <div className="flex items-center gap-2 px-4 py-2 bg-zinc-100 rounded-xl border border-zinc-300">
                <span className="text-xs font-manga text-zinc-600">
                  PDF Quality:
                </span>
                <button
              ...
              <button
                ...
                title={
                  exportPages.length === 0
                    ...
                    : `Download ${exportPages.length} pages as PDF (${pdfQuality.toUpperCase()} quality)`
                }
              >
                <Download size={20} />
                <span>
            ...
            <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
              <div className="bg-white rounded-xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col">
                <div className="p-6 border-b border-zinc-200 flex justify-between items-center">
                  <h2 className="text-2xl font-manga text-black">
                    Add Images from Other Sessions
                  </h2>
                  <button
                      ...
                      .map((session) => (
                        ...
                        >
                          <div className="flex items-center gap-2 mb-4">
                            ...
                            <h3 className="text-lg font-manga text-black">
                              {session.name}
                            </h3>
                            <span className="text-sm text-zinc-500">
                              ({session.pages.length} images)
                            </span>
                          </div>
                                  ...
                                  onClick={async () => {
                                    if (currentSession) {
                                      ...
                                      try {
                                        ...
                                        console.error(
                                          'Failed to add page to session on backend',
                                          err,
                                        );
                                      }
                                      setProject(updatedProject);
                          ...
                          {session.pages.length === 0 && (
                            <p className="text-center text-zinc-400 font-manga py-4">
                              No images in this session
                            </p>
                          )}
                        </div>
                      ))}
                    {project.sessions.filter((s) => s.id !== currentSession?.id)
                      .length === 0 && (
                      <p className="text-center text-zinc-400 font-manga py-8">
          ...
          {currentSession && currentSession.pages.length > 0 && (
            <div className="print:hidden mb-12">
              <h2 className="text-2xl font-manga text-black mb-4">
                All Images in Current Session
              </h2>
              <p className="text-sm text-zinc-500 mb-6 font-manga">
                Click on images to toggle PDF export mark (✓ = will be included
                in PDF)
              </p>
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
...
export function PreviewPage(_props: any) {
  return (
    <Suspense
      fallback={

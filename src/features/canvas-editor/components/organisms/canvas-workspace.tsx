  ...
  const handleCanvasClick = (e: ReactMouseEvent) => {
    if (
      ...
      if (tool === 'panel') {
        ...
        const pos = getMousePosition(e);
        const newDialogue = createDialogueElement(
          generateId(),
          'Dialogue...',
          pos,
        );
  ...
  const renderElement = (element: CanvasElement) => {
    ...
    const renderContent = () => {
      switch (element.type) {
          ...
          return element.src ? (
            <div className="relative w-full h-full">
              <Image
                ...
                style={{
                  objectFit: element.objectFit as any, // Cast to any because generic CSSProperties objectFit might mismatch with Image props roughly? No, Image style prop accepts standard CSS. But wait, element.objectFit is likely 'contain' | 'cover' etc.
                  opacity: element.opacity,
                }}
                draggable={false}
                sizes={`${(currentPage?.width || 800) * zoom}px`} // Approximate size... actually element has specific size.
              />
            </div>
          ) : (
        ...
        case 'dialogue':
          return <DialogueBubble element={element} />;
    ...
    return (
      <div
        ...
        className={cn(
          'group',
          isSelected &&
            'ring-2 ring-purple-500 ring-offset-1 ring-offset-transparent',
        )}
          ...
          <>
            ...
            {['nw', 'ne', 'se', 'sw'].map((handle) => (
              <div
                ...
                onMouseDown={(e) =>
                  handleResizeMouseDown(e, element.id, handle)
                }
              />
            ))}
            {/* Edges */}
            {['n', 'e', 's', 'w'].map((handle) => (
              <div
                ...
                className={cn(
                  ...
                  handle === 'n' &&
                    'w-3 h-3 top-[-6px] left-1/2 -translate-x-1/2 cursor-n-resize',
                  handle === 's' &&
                    'w-3 h-3 bottom-[-6px] left-1/2 -translate-x-1/2 cursor-s-resize',
                  handle === 'e' &&
                    'w-3 h-3 right-[-6px] top-1/2 -translate-y-1/2 cursor-e-resize',
                  handle === 'w' &&
                    'w-3 h-3 left-[-6px] top-1/2 -translate-y-1/2 cursor-w-resize',
                )}
                onMouseDown={(e) =>
                  handleResizeMouseDown(e, element.id, handle)
                }
...
import { DialogueBubble } from '../molecules/dialogue-bubble';

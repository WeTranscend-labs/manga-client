...
export function ProjectDetailsPage(_props: any) {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        const [projectData, likesData, commentsData, relatedData] =
          await Promise.all([
            storageService.fetchPublicProjectDetail(
              params.ownerId,
              params.projectId,
              true,
            ), // Track view
            communityService.fetchLikes(params.ownerId, params.projectId),
            communityService.fetchComments(params.ownerId, params.projectId),
            communityService.fetchRelatedProjects(
              params.ownerId,
              params.projectId,
              6,
            ),
  ...
  if (loading) {
    return (
      <Page className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 flex items-center justify-center">
  ...
  if (!project) {
    return (
      <Page className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-4">
        <button
          onClick={() => router.back()}
        ...
        <div className="text-zinc-400 text-sm">
          Không tìm thấy tập truyện hoặc tập truyện đã bị ẩn.
        </div>
      </Page>
    );
  }
  ...
  const handleToggleLike = async () => {
    ...
    const res = await communityService.toggleLike(
      params.ownerId,
      params.projectId,
    );
  ...
  const renderComments = (parentId: string | null, depth = 0) => {
    ...
    return (
      <div
        className={
          depth === 0 ? 'space-y-3' : 'space-y-3 pl-4 border-l border-zinc-800'
        }
      >
        {list.map((comment) => (
          <div
            key={comment.id}
            className="rounded-xl bg-zinc-900/60 border border-zinc-800 px-3 py-2"
          >
  ...
  return (
    <Page className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      <button
        onClick={() => router.back()}
        className="text-xs text-zinc-400 hover:text-zinc-200 underline underline-offset-4"
      >
          ...
          <div className="flex items-center gap-4 pt-2">
            ...
            >
              <Heart
                size={14}
                className={
                  likedByUser ? 'fill-red-500 text-red-500' : 'text-zinc-400'
                }
              />

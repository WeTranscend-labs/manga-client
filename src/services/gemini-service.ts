...
export const generateNextPrompt = async (
  ...
  const allPreviousPrompts = sessionHistory
    .map((p) => p.prompt)
    .filter((p) => p && p.trim());
  const recentPrompts = recentPages
    .map((p) => p.prompt)
    .filter((p) => p && p.trim());
...
${
  sessionHistory.length > 0
    ? `⚠️ CRITICAL: Analyze what happened in Page ${sessionHistory.length} (the MOST RECENT page) and write a prompt for what happens NEXT.
...
Your prompt should describe the NEXT scene that continues chronologically from Page ${sessionHistory.length}.`
    : 'Write a prompt for the opening scene of this manga story.'
}
...
${
  layout &&
  panelCountRequirement.includes('PANEL') &&
  !panelCountRequirement.includes('SINGLE')
    ? `
  ...
  try {
    ...
    while (true) {
      try {
        ...
        console.warn(
          `Gemini text model overloaded, retrying in ${backoffMs}ms (attempt ${attempts}/${maxAttempts})`,
        );
    ...
    if (allPreviousPrompts.length > 0 && generatedPrompt) {
      ...
      while (retryCount < maxRetries) {
        ...
        const isTooSimilar = allPreviousPrompts.some((prevPrompt) => {
          const similarity = calculatePromptSimilarity(
            generatedPrompt.toLowerCase(),
            prevPrompt.toLowerCase(),
          );
        ...
        retryCount++;
        console.warn(
          `⚠️ Generated prompt is too similar to previous prompts. Retrying (${retryCount}/${maxRetries})...`,
        );
...
- Make it DISTINCT and UNIQUE from all previous prompts listed above.`;
      ...
      if (retryCount >= maxRetries) {
        console.warn(
          '⚠️ Could not generate completely unique prompt after retries, but proceeding with generated prompt.',
        );
      }
    }
...
export const generateMangaImage = async (
  ...
  if (hasPreviousPages) {
    ...
    isBatchContinuation = cleanedPrompt.includes(
    ...
    // PRIORITY 2: Batch continuation (auto-generated)
      ...
      const pageMatch = prompt.match(
        /page (\d+)\. This is page (\d+) of (\d+)/,
      );
      if (pageMatch) {
...
Create the NEXT scene that follows from previous pages. Change: camera angle, character pose, composition. Advance narrative naturally.`;
    ...
    // BUT: Still prioritize story direction and context if available
      ...
      (!cleanedPrompt ||
        cleanedPrompt.trim() === '' ||
        cleanedPrompt === 'Continue the story naturally' ||
...
Language: ${config.language.toUpperCase()} only`;
  ...
  const getStyleDescription = (style: string) => {
    const styleGuides: Record<string, string> = {
      'Modern Webtoon':
        'Modern Korean WEBTOON style: vertical-reading composition, clean polished digital rendering, vibrant pastel colors, soft shading, thin elegant line art, subtle gradients, cinematic lighting, focus on faces and emotions.',
      'Korean Manhwa':
        'Traditional Korean MANHWA style: semi-realistic proportions, detailed faces, sharp but elegant line art, rich color palettes, dramatic lighting, fashion-focused clothing, smooth gradient shading.',
      'Manhwa 3D':
        'MANHWA 3D style: three-dimensional rendering with depth and volume, semi-realistic proportions with 3D modeling aesthetics, detailed faces with dimensional lighting, sharp but elegant line art, rich vibrant color palettes, dramatic three-dimensional lighting with shadows and highlights, fashion-focused clothing with 3D texture rendering, SMOOTH GLOSSY SHINY POLISHED rendering with silky smooth surfaces, reflective glossy materials, sleek polished skin and hair, smooth gradient shading with volumetric effects and glossy highlights, ultra-smooth bóng mượt finish, cinematic depth of field, realistic perspective and camera angles, professional 3D game/animation quality rendering with glossy polished surfaces.',
      'Digital Painting':
        'Full DIGITAL PAINTING style: painterly brush strokes, visible texture, blended edges, strong light and shadow shapes, rich color harmonies, atmospheric perspective, soft but detailed rendering.',
      'Realistic Manga':
        'REALISTIC manga style: accurate human anatomy, realistic proportions, detailed facial features, strong three-dimensional shading, subtle screentones, grounded camera angles while keeping manga aesthetics.',
      'Realistic People':
        'REALISTIC PEOPLE style: photorealistic human anatomy and proportions, lifelike facial features and expressions, natural skin textures and hair, realistic lighting and shadows, authentic clothing and environments, true-to-life body language and poses, photographic quality rendering with detailed facial features, realistic eye reflections, natural skin imperfections, authentic material textures, cinematic depth of field, professional photography aesthetics.',
      'Clean Line Art':
        'CLEAN LINE ART style: crisp vector-like outlines, uniform line weight or very controlled variation, minimal hatching, flat or very simple shading, graphic and modern look with clear silhouettes.',
      'Cinematic Style':
        'CINEMATIC style: movie-like framing, extreme close-ups and wide shots, dramatic contrast lighting, depth of field blur, lens effects, dynamic perspective, very atmospheric backgrounds.',
      'Semi-Realistic':
        'SEMI-REALISTIC style: balance between anime and realism, believable anatomy but stylized faces, detailed rendering of hair and skin, controlled line work, soft but accurate lighting.',
      Shonen:
        'SHONEN manga style: high-energy action, exaggerated expressions, strong dynamic poses, thick energetic line art, speed lines, impact frames, high contrast screentones, youthful character designs.',
      Shoujo:
        'SHOUJO manga style: delicate graceful line art, large sparkling eyes, thin flowing hair, elegant poses, decorative screentones (flowers, sparkles), soft shading, focus on romance and emotion.',
      Seinen:
        'SEINEN manga style: mature and detailed, realistic body proportions, complex hatching and screentones, gritty textures, grounded camera angles, subtle but powerful expressions.',
      Josei:
        'JOSEI manga style: adult slice-of-life tone, realistic but gentle proportions, fashionable clothing, clean refined line art, soft screentones, nuanced facial expressions and body language.',
    };
  ...
  const getInkingDescription = (inking: string) => {
    const inkingGuides: Record<string, string> = {
      'Digital Painting':
        'Full digital painting with blended colors, no hard line art, painterly texture and brushwork',
      'Soft Brush':
        'Soft, organic brush strokes with gentle edges and smooth transitions',
      'Clean Digital':
        'Precise, clean digital lines with consistent weight and smooth curves',
      Airbrush:
        'Smooth airbrush shading with soft gradients and subtle color transitions',
      Painterly:
        'Expressive painterly strokes with visible brush texture and artistic flair',
      'G-Pen':
        'Traditional manga G-pen with variable line weight, crisp blacks',
  ...
  const adjustPromptWithAI = async (
    originalPrompt: string,
    blockReason: string,
    attempt: number,
  ): Promise<string> => {
    try {
      const ai = new GoogleGenAI({
        apiKey:
          process.env.API_KEY || 'AIzaSyCWdZeeNGdHbRGqoisSNI4_nj2hHpCQqiI',
      });
...
Return ONLY the rewritten prompt, nothing else. No explanations, no meta-commentary.`;
      ...
      if (adjustedPrompt && adjustedPrompt.length > 10) {
        ...
        console.warn(
          `⚠️ AI adjustment failed, falling back to sanitize method`,
        );
  ...
  const sanitizeEnhancedPromptForRetry = (
  ...
  if (config.language === 'English') {
    ...
    priorities.push(
      '1. Vietnamese text accuracy (all diacritics correct, verify "đ" vs "d")',
    );
  } else {
  ...
  const hasSpacing =
    config.panelBorderStyle === 'Full Border' || !config.panelBorderStyle; // Default to spacing
  ...
  try {
    ...
    if (sessionHistory && sessionHistory.length > 0) {
      ...
      if (selectedReferencePageIds && selectedReferencePageIds.length > 0) {
        ...
        pagesToUse = sessionHistory.filter((page) =>
          selectedReferencePageIds.includes(page.id),
        );
        console.log(
          `📸 Using ${pagesToUse.length} user-selected reference pages:`,
        ...
        console.log(
          `📸 Using last 3 pages as reference (no specific selection)`,
        );
      }
    ...
    while (retryAttempt <= maxRetries) {
      try {
        ...
        if (retryAttempt > 0 && !aiAdjusted) {
          console.warn(
            `🔄 Retry attempt ${retryAttempt}/${maxRetries}: Modifying prompt to be less explicit...`,
          );
          const sanitizedEnhancedPrompt = sanitizeEnhancedPromptForRetry(
            enhancedPrompt,
            actualPrompt,
            retryAttempt,
          );
          currentActualPrompt = sanitizePromptForRetry(
            actualPrompt,
            retryAttempt,
          );
        ...
        while (true) {
          try {
            ...
            innerAttempts += 1;
            if (
              !isOverloadedError(overErr) ||
              innerAttempts >= innerMaxAttempts
            ) {
              throw overErr;
            }
        ...
        if (response.promptFeedback?.blockReason) {
          if (
            ...
            console.warn(
              `⚠️ Attempt ${retryAttempt + 1} blocked: PROHIBITED_CONTENT (promptFeedback). Using AI to adjust prompt...`,
            );
            retryAttempt++;
            lastError = new Error(
              `Content blocked: ${response.promptFeedback.blockReason}. ${response.promptFeedback.blockReasonMessage || ''}`,
            );
            ...
            try {
              ...
              const userPromptSectionMatch = enhancedPrompt.match(
                /WRITE YOUR PROMPT \(USER INPUT\):\n(.*?)(?=\n|$)/s,
              );
              ...
              if (userPromptSectionMatch) {
                ...
                const storySectionMatch = enhancedPrompt.match(
                  /STORY CONTINUATION:\n(.*?)(?=\n[A-Z])/s,
                );
              ...
              currentContentParts = [
                { text: newEnhancedPrompt },
                ...contentParts.slice(1),
              ];
              aiAdjusted = true; // Mark that AI adjustment was used

              console.log(
                `🔄 Retry attempt ${retryAttempt}/${maxRetries}: Using AI-adjusted prompt`,
              );
            } catch (adjustError) {
              console.error(
                'Error adjusting prompt with AI, falling back to sanitize:',
                adjustError,
              );
            ...
            if (response.promptFeedback.blockReason === 'PROHIBITED_CONTENT') {
              console.warn(
                `Content was blocked as PROHIBITED_CONTENT after ${retryAttempt} retry attempts.`,
              );
              console.warn(
                'Note: Even with safety settings disabled, Gemini API may still block certain content types.',
              );
              console.warn(
                'Consider using alternative APIs or models that support adult content generation.',
              );
              const error = new Error(
                `Content blocked after ${retryAttempt} attempts: ${response.promptFeedback.blockReason}. ${response.promptFeedback.blockReasonMessage || "The prompt violated Google's content policy."}`,
              );
        ...
        if (
          ...
          console.warn(
            `⚠️ Attempt ${retryAttempt + 1} blocked: PROHIBITED_CONTENT (finishReason). Using AI to adjust prompt...`,
          );
          retryAttempt++;
          lastError = new Error(
            `Content blocked: PROHIBITED_CONTENT. ${candidate.finishMessage || "The image violated Google's content policy."}`,
          );
        ...
        if (
          ...
          console.warn(
            `⚠️ Attempt ${retryAttempt + 1} blocked: IMAGE_SAFETY (finishReason). Using AI to adjust prompt...`,
          );
          retryAttempt++;
          lastError = new Error(
            `Image blocked by safety filter (IMAGE_SAFETY): ${candidate.finishMessage || "The image violated Google's content policy."}`,
          );
        ...
        if (candidate.finishReason && candidate.finishReason !== 'STOP') {
          ...
          if (candidate.finishReason === 'IMAGE_SAFETY') {
            console.error(
              `Image was blocked by IMAGE_SAFETY filter after ${retryAttempt} retry attempts`,
            );
            console.warn(
              "The generated image violated Google's Generative AI Prohibited Use policy",
            );
            console.warn(
              "This can happen even with safety settings disabled due to Google's content policy",
            );
            console.warn('Suggestions:');
            console.warn('1. Try rephrasing the prompt to be less explicit');
            console.warn('2. Use more artistic/abstract descriptions');
            console.warn(
              '3. Consider using alternative APIs that support adult content',
            );
            const error = new Error(
              `Image blocked by safety filter (IMAGE_SAFETY) after ${retryAttempt} attempts: ${candidate.finishMessage || "The image violated Google's content policy. Try rephrasing the prompt or using alternative APIs."}`,
            );
            (error as any).retryCount = retryAttempt;
            (error as any).maxRetries = maxRetries;
            throw error;
          ...
          throw new Error(
            `Generation stopped: ${candidate.finishReason}. ${candidate.finishMessage || ''}`,
          );
        }

        // Success - break out of retry loop
        console.log(
          `✅ Generation successful${retryAttempt > 0 ? ` after ${retryAttempt} retry attempt(s)` : ''}`,
        );
        break;
      } catch (error: any) {
        ...
        if (
          ...
          const errorType = error.message?.includes('IMAGE_SAFETY')
            ? 'IMAGE_SAFETY'
            : 'PROHIBITED_CONTENT';
          console.warn(
            `⚠️ Attempt ${retryAttempt + 1} failed: ${errorType}. Using AI to adjust prompt...`,
          );
          retryAttempt++;
          lastError = error;
    ...
    if (lastError && !response) {
      const error = new Error(
        `Failed to generate after ${retryAttempt} attempts: ${lastError.message}`,
      );
      (error as any).retryCount = retryAttempt;
      (error as any).maxRetries = maxRetries;
      throw error;
    ...
    if (!response) {
      const error = new Error(
        `Failed to generate content after ${retryAttempt} retry attempts`,
      );
      (error as any).retryCount = retryAttempt;
      (error as any).maxRetries = maxRetries;
      throw error;
    ...
    if (candidate.finishReason && candidate.finishReason !== 'STOP') {
      ...
      if (
        ...
        console.warn(
          `Content was blocked after ${retryAttempt} retry attempts.`,
        );
        console.warn(
          'Note: Even with safety settings disabled, Gemini API may still block certain content types.',
        );
        ...
        const error = new Error(
          `Generation stopped after ${retryAttempt} attempts: ${candidate.finishReason}. ${candidate.finishMessage || "The image violated Google's content policy."}`,
        );
        (error as any).retryCount = retryAttempt;
        (error as any).maxRetries = maxRetries;
        throw error;
    ...
    throw new Error(
      'No image data returned from Gemini - check console for details',
    );
  } catch (error) {
